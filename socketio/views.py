...
def save_message(request):
    if request.method == 'POST':
        ...
        try:
            ...
            if not all(v is not None for v in [user_id, login_type, business_id, node_id, role_alloc_id]):
            ...
            if business is None:
                ...
                return HttpResponse(json.dumps(resp, ensure_ascii=False), content_type="application/json")
            bra = BusinessRoleAllocation.objects.filter(pk=role_alloc_id, business_id=business_id, project_id=business.cur_project_id).first()
            if bra is None:
            ...
            if path is None:
                ...
                return HttpResponse(json.dumps(resp, ensure_ascii=False), content_type="application/json")
            brat = BusinessRoleAllocationStatus.objects.filter(business_role_allocation_id=role_alloc_id, business_id=business_id, path_id=path.pk).first()
            # 是否有结束环节的权限
            ...
            bps = BusinessPositionStatus.objects.filter(business_id=business_id, business_role_allocation_id=role_alloc_id, path_id=path.id).first()
            if bps:
            ...
            message = None
            if role_alloc_id:
                message = BusinessMessage.objects.create(business_id=business_id, user_id=user_id, business_role_allocation_id=role_alloc_id,
