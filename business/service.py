...
def action_role_meet(bus, node_id, path_id, role):
    """
    申请约见
    :param role_id: 角色id
    :param node_id: 环节id
    ...
    """
    try:
        BusinessRoleAllocationStatus.objects.filter(business_id=bus.id, node_id=node_id, path_id=path_id, role_id=role.id).update(come_status=1, stand_status=2)
